...
class NetWorkBangumiAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder.itemViewType == BANNER_VIEWTYPE) {
            ...
            if (recyclerView.adapter == null) {
                recyclerView.adapter = BannerAdapter().apply { submitList(homeBangumis[BANNER]) }
            } else {
    ...
    inner class ViewHolder(
        ...
        fun bind(title: String) {
            ...
            //暂时想不到其他办法
            bind.homebangumiTitle.setOnClickListener {
                val activity = it.context as MainActivity
                val directions = HomeFragmentDirections.
                        actionHomeFragmentToCategoryDetailFragment(title)
